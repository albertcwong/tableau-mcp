...
export const getPublishWorkbookTool = (server: Server): Tool<typeof paramsSchema> => {
  const tool = new Tool({
    ...
    description:
      'Publishes a workbook to a project. Use when the agent needs to publish a .twbx file. Requires projectId, name, and either contentBase64 (inline) or uploadSessionId (for large files). Overwrite=true to replace existing same-name workbook. Returns published workbook metadata.',
    paramsSchema,
      ...
      if (!contentBase64 && !uploadSessionId) {
        return {
          ...
          content: [
            {
              type: 'text',
              text: 'One of contentBase64 or uploadSessionId is required. For files >64MB use upload session.',
            },
          ],
        };
      }
      ...
      return await tool.logAndExecute<Record<string, string>>({
        ...
        callback: async () => {
          ...
          const meta = await useRestApi({
            ...restApiArgs,
            jwtScopes: ['tableau:workbooks:create'],
            callback: (api) =>
