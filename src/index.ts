...
// Keep the process alive on unexpected errors rather than crashing the MCP server.
process.on('uncaughtException', (error) => {
  log({ message: 'Uncaught exception', level: 'error', logger: 'process', data: error });
});

process.on('unhandledRejection', (reason) => {
  log({ message: 'Unhandled rejection', level: 'error', logger: 'process', data: reason });
});
